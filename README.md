...
DynStav 是一款 **Windows 桌面应用**（基于 Electron + React + TypeScript），用于复盘时，观察同花顺中自定义动态板块的趋势变化。

本韭菜觉得同花顺条件选股很好用，而且可以保存为动态板块，每天按照选股条件筛选出一些股票，但暂时没找到可以跟踪这个动态股票池趋势变化的方式，所以用AI搞了这个应用。
...
- 本项目**只读取**（绝不修改）用户本机上同花顺客户端的配置文件 `stockblock.ini`，用于识别用户自己定义的动态板块。所有数据所有权归用户本人。
- 使用本软件产生的一切后果由使用者自行承担。

### 关于使用

- 本软件的分析数据完全依赖同花顺动态板块的每日更新。
- 连续使用多天、每日坚持同步后，趋势图表才会逐渐呈现出完整的走势变化，届时分析结果才更具参考价值。
...
- 🤖 **AI 板块分析**：右侧 AI 对话面板，基于当前板块的真实行情数据进行智能分析，支持 OpenAI / Azure / Anthropic / 自定义模型接口。内置 13 家主流 AI 服务商预设，填入 API Key 即可一键获取最新模型列表，每个供应商可配置多个模型并独立设置参数，输入区一键切换。
...
![选择主题](resources/theme.png)
![选择配置](resources/config.png)
![暗色主题](resources/dark.png)
![亮色主题](resources/light.png)
![大模型配置](resources/ai-config.png)
